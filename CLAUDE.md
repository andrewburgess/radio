...
- **Concurrency**: use channels for inter-goroutine communication; avoid shared
  mutable state where possible; protect shared state with `sync.Mutex` when necessary.
- **Subprocess management**: use `os/exec`; always capture stderr, always handle
  process death and restart.
- **Config**: environment variables only, loaded at startup via a `config` package.
...
Apply schema via embedded SQL in `store/store.go` on first open (`CREATE TABLE IF NOT EXISTS`).
